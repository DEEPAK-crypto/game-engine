...
/**
 * Quick Smoke Test Simulation for Game Platform (Kotlin)
 *
 * Fast validation test with 100 users and 3 questions.
 * Use this for quick testing during development.
 *
 * Run with: ./gradlew gatlingRun-com.gameplatform.game.gatling.QuickSmokeTestSimulation
 */
class QuickSmokeTestSimulation : Simulation() {

    // Configuration
    private val baseUrl = System.getProperty("baseUrl", "http://localhost:8080")
    private val numQuestions = 3
    private val questionTimer = 10
    private val questionReward = 100.0
    private val targetUsers = 100
    ...
    private val httpProtocol = http
        ...
        .userAgentHeader("Gatling Quick Smoke Test - Kotlin")
    ...
    private val setupScenario = scenario("Setup Game")
        .exec(
            http("Create Game")
                ...
                .body(StringBody("""
                    {
                        "name": "Quick Smoke Test - $targetUsers Users",
                        "gameType": "MCQ_FIFO",
            ...
            http("Add Questions")
                ...
                .body(StringBody { session ->
                    val questions = (1..numQuestions).joinToString(",\n", "[", "]") { i ->
                        """
                        {
                            "questionText": "Quick Test Question $i - What is the answer?",
                            "options": [
        ...
        .exec { session ->
            ...
            println("\n========================================")
            println("Starting quick smoke test with $targetUsers users")
            println("========================================\n")
            session
        }
    ...
    private val teardownScenario = scenario("Teardown")
        ...
        .exec { session ->
            println("\n========================================")
            println("Quick smoke test completed")
            println("========================================\n")
            session
        }
        .exec(
            http("Get Final Leaderboard")
                .get("/api/leaderboards/games/#{gameId}?limit=10")
                .check(status().shouldBe(200))
        ...
        .exec { session ->
            ...
            println("\n========================================")
            println("Quick Smoke Test Complete!")
            println("========================================\n")
            session
        }
    ...
    init {
        setUp(
            ...
            setupScenario.injectOpen(atOnceUsers(1))
                .andThen(
                    // Then: Quickly ramp up users
                    playGameScenario.injectOpen(
                        rampUsers(50).during(Duration.ofSeconds(10)),   // 0 → 50 users over 10s
                        rampUsers(50).during(Duration.ofSeconds(10)),   // 50 → 100 users over 10s
                        constantUsersPerSec(10.0).during(
         ...
         .assertions(
             ...
             global().responseTime().percentile4().lt(2000),  // 99th percentile < 2s
             global().successfulRequests().percent().gt(85.0) // > 85% success rate
         )
    }
}